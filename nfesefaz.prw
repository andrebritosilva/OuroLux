...
±±³Descri‡…o ³Rdmake de- exemplo para geracao da Nota Fiscal Eletronica   ³±±
				...
				Else
					...
					SF2->F2_DOC == (cAliasSD2)->D2_DOC
						...
						If lContinua
							...
							If Empty(aPedido)
								// MOA - 25/09/2019 - 08:45hs - Leroy Merlin
								// aPedido := {Iif(SC5->(FieldPos("C5_NTEMPEN")) > 0,Alltrim(SC5->C5_NTEMPEN),""),AllTrim(SC6->C6_PEDCLI),""}
								aPedido := {Iif(SC5->(FieldPos("C5_NTEMPEN")) > 0,Alltrim(SC5->C5_NTEMPEN),""),AllTrim(SC6->C6_NUMPCOM),""}
							EndIf
