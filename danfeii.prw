...
private oRetNF
Aviso("DANFE II","Compatibilização Protheus 12 - 17/07/2018",{"Ok"})
If nTipo <> 1
...
// Inicio Customização Ourolux
If (cFilAnt == '01' .AND. !(MV_PAR03 $ '5  .555')) .Or. (cFilAnt == '02' .AND. !(MV_PAR03 $ '4  .900.444')) .Or. (cFilAnt == '03' .AND. !(MV_PAR03 $ '6  .666')) .Or. (cFilAnt == '04' .AND. !(MV_PAR03 $ '7  .777')) .Or. (cFilAnt == '05' .AND. !(MV_PAR03 $ '8  .888'))      
	 Set Device To Screen
	 ApMsgStop( 'Serie da Nota Fiscal errada. Digite a serie correta!', 'ATENÇÃO' )
	 Return
EndIf
// Fim Customização Ourolux
...
// INICIO CUSTOMIZACAO OUROLUX CLAUDINO
For nX := 1 To nLenDet
	Aadd(aIndAux, {nX, SubStr(NoChar(oDet[nX]:_Prod:_cProd:TEXT,lConverte),1,MAXITEMC)})
Next
...
For nX := 1 To nLenDet
	Aadd(aIndImp, aIndAux[nX][1] )
Next
// FIM CUSTOMIZACAO OUROLUX CLAUDINO
...
// Customizacao Ourolux - Isaias Alterao de 9 quadros(parcelas) 12
nposrod:=000
nposright:=050

for npos:=1 to 12  

oDanfe:Box(265,nposrod,296,nposright) 
...
next
/*
oDanfe:Box(265,047,296,095)
oDanfe:Box(265,94,296,142)
oDanfe:Box(265,141,296,189)
...
nX:=0   // Customizado Isaias 
nColuna := 002
If Len(aFaturas) >0
	nMaxFat := Len(aFaturas[01])
	For nY := 1 To Min(nFaturas,nMaxFat)
	    nX++
		oDanfe:Say(273,nColuna,aFaturas[nX][nY],oFont08:oFont)
		nX++
		oDanfe:Say(281,nColuna,aFaturas[nX][nY],oFont08:oFont)
		nX++
		oDanfe:Say(289,nColuna,aFaturas[nX][nY],oFont08:oFont)
		
		nColuna:= nColuna+50 //67 Customizacao Ourolux - Isaias - Alterado coluna onde a informacao é escrita.
		nX:=0    
	Next nY
Endif
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
